...
public class SimpleHtmlTemplateTests
{
    ...
    {
        var title = "Title";
        var result = Html.Template($"<h1>{title}</h1>").ToString();
    ...
    public void Render_HtmlContent_Escapes()
    {
        ...
        var result = Html.Template($"<div>{userInput}</div>").ToString();
    ...
    {
        string? value = null;
        var result = Html.Template($"<span>{value}</span>").ToString();
    ...
    public void Render_MultipleArguments()
    {
        var title = "Hello";
        var body = "World";
        var result = Html.Template($"<h1>{title}</h1><p>{body}</p>").ToString();
